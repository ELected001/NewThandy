...
import { createPageMetadata, createSitePageMetadata } from "@/lib/metadata";
import {
  createFaqSchema,
...
describe("SEO contract", () => {
  it("keeps configured page titles and descriptions unique", () => {
  ...
  it("keeps configured meta descriptions within search snippet guidance", () => {
  ...
  it("builds metadata from the code-managed page configuration", () => {
    const metadata = createSitePageMetadata("blog");

    expect(metadata.title).toBe(seo.pages.blog.title);
    expect(metadata.description).toBe(seo.pages.blog.description);
